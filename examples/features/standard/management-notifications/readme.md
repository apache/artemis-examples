...
This example shows how to receive management notifications from Apache Artemis using JMS Messages.

Apache Artemis servers emit management notifications when events of interest occur (consumers are created or closed, destinations are created or deleted, security authentication fails, etc.).
...
Apache Artemis can configured to send JMS messages when management notifications are emitted on the server.
