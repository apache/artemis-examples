...
This example shows you how to implement and configure a simple incoming, server-side MQTT interceptor with Apache Artemis.

Apache Artemis allows an application to use an interceptor to hook into the messaging system. To intercept MQTT packets all that needs to be done is to implement the `org.apache.activemq.artemis.core.protocol.mqtt.MQTTInterceptor` interface.
