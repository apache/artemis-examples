...
This example shows you how to configure Apache Artemis so messages are expipired after a certain time.
...
Apache Artemis can assign a _expiry address_ to a given queue so that when messages are expired, they are removed from the queue and routed to this address. These "expired" messages can later be consumed for further inspection.
